...
app.post('/api/auth/register', (req, res) => {
  try {
    ...
    if (existingUser) {
      securityMonitor.addSuspiciousActivity(email, req.clientIP, 'DUPLICATE_EMAIL', {});
      return res.status(400).json({
        success: false,
...
app.post('/api/auth/logout', verifyToken, (req, res) => {
  try {
    ...
    activityLogger.log(userId, 'LOGOUT', { ip: req.clientIP }, req.fingerprint.hash);
...
app.get('/api/withdrawal/requests', verifyToken, (req, res) => {
  try {
    const userId = req.user.userId;
    const userWithdrawals = Object.values(withdrawalRequests).filter(w => w.userId === userId);
...
app.post('/api/payment/submit-subscription', verifyToken, (req, res) => {
  try {
    ...
    const sanitizedAmount = typeof amount === 'string' ? amount.replace(/[\n\r]/g, '') : amount;
    const sanitizedUserId = typeof userId === 'string' ? userId.replace(/[\n\r]/g, '') : userId;

    console.log(`💳 Payment submission from user ${sanitizedUserId}`, { txHash: sanitizedTxHash, amount: sanitizedAmount });
    ...
    const paymentId = Math.max(0, ...Object.keys(paymentRecords).map(Number)) + 1;
    const payment = {
      id: paymentId,
      userId,
      amount,
    ...
    console.log(`✅ Payment completed for user ${sanitizedUserId}`, sanitizedPaymentForLog);

    res.json({
      success: true,
      message: 'Payment verified successfully! You now have access to the VIP channel.',
      payment: {
        id: paymentId,
        status: 'completed',
...
app.get('/api/subscription/status', verifyToken, (req, res) => {
  try {
    const userId = req.user.userId;
    const user = users[userId];
    const userPayments = Object.values(paymentRecords).filter(p => p.userId === userId);
    const hasPaid = userPayments.length > 0 && userPayments[userPayments.length - 1].status === 'completed';
...
app.post('/api/admin/withdrawal/approve/:withdrawalId', verifyToken, adminOnly, (req, res) => {
  try {
...
app.post('/api/admin/withdrawal/reject/:withdrawalId', verifyToken, adminOnly, (req, res) => {
  try {
    const { withdrawalId } = req.params;
...
app.get('/api/admin/users', (req, res) => {
  res.json({
    success: true,
    users: [
      { id: 1, name: 'Ahmed Hassan', email: 'ahmed@example.com', status: 'Active', joinDate: '2024-12-01', balance: 5000, referrals: 12 },
      { id: 2, name: 'Fatima Ali', email: 'fatima@example.com', status: 'Active', joinDate: '2024-12-05', balance: 3200, referrals: 8 },
      { id: 3, name: 'Mohamed Ibrahim', email: 'mohamed@example.com', status: 'Active', joinDate: '2024-12-10', balance: 7500, referrals: 15 },
      { id: 4, name: 'Layla Ahmed', email: 'layla@example.com', status: 'Inactive', joinDate: '2024-12-12', balance: 1200, referrals: 3 },
      { id: 5, name: 'Hassan Karim', email: 'hassan@example.com', status: 'Active', joinDate: '2024-12-15', balance: 4500, referrals: 10 }
    ],
    total: 156,
    active: 89
  });
...
app.get('/api/admin/payments', (req, res) => {
  res.json({
    success: true,
    payments: [
      { id: 1, userId: 1, userName: 'Ahmed Hassan', amount: 500, method: 'Bank Transfer', status: 'Completed', date: '2024-12-20', reference: 'PAY-001' },
      { id: 2, userId: 2, userName: 'Fatima Ali', amount: 300, method: 'Cryptocurrency', status: 'Completed', date: '2024-12-21', reference: 'PAY-002' },
      { id: 3, userId: 3, userName: 'Mohamed Ibrahim', amount: 1000, method: 'Bank Transfer', status: 'Pending', date: '2024-12-22', reference: 'PAY-003' },
      { id: 4, userId: 4, userName: 'Layla Ahmed', amount: 200, method: 'Cryptocurrency', status: 'Failed', date: '2024-12-23', reference: 'PAY-004' },
      { id: 5, userId: 5, userName: 'Hassan Karim', amount: 750, method: 'Bank Transfer', status: 'Completed', date: '2024-12-24', reference: 'PAY-005' }
    ],
    total: 45320.50,
    completed: 32450,
    pending: 8900,
...
app.get('/api/admin/withdrawals', (req, res) => {
  res.json({
    success: true,
    withdrawals: [
      { id: 1, userId: 1, userName: 'Ahmed Hassan', amount: 2000, method: 'Bank Transfer', status: 'Completed', requestDate: '2024-12-20', processedDate: '2024-12-21' },
      { id: 2, userId: 2, userName: 'Fatima Ali', amount: 1500, method: 'Cryptocurrency', status: 'Completed', requestDate: '2024-12-21', processedDate: '2024-12-22' },
      { id: 3, userId: 3, userName: 'Mohamed Ibrahim', amount: 3000, method: 'Bank Transfer', status: 'Pending', requestDate: '2024-12-25', processedDate: null },
      { id: 4, userId: 4, userName: 'Layla Ahmed', amount: 500, method: 'Cryptocurrency', status: 'Pending', requestDate: '2024-12-26', processedDate: null },
      { id: 5, userId: 5, userName: 'Hassan Karim', amount: 2500, method: 'Bank Transfer', status: 'Completed', requestDate: '2024-12-24', processedDate: '2024-12-25' }
    ],
    total: 12500,
    completed: 9500,
    pending: 3200
...
app.get('/api/admin/logs', verifyToken, adminOnly, (req, res) => {
  res.json({
    success: true,
    logs: [
      { id: 1, timestamp: '2024-12-29 10:30:45', user: 'Ahmed Hassan', action: 'Login', details: 'Successful login from 192.168.1.1', type: 'LOGIN' },
      { id: 2, timestamp: '2024-12-29 10:35:20', user: 'Fatima Ali', action: 'Payment Submitted', details: 'Payment of 500 USD submitted', type: 'PAYMENT' },
      { id: 3, timestamp: '2024-12-29 10:40:15', user: 'Mohamed Ibrahim', action: 'Withdrawal Request', details: 'Withdrawal of 3000 USD requested', type: 'WITHDRAWAL' },
      { id: 4, timestamp: '2024-12-29 10:45:30', user: 'Layla Ahmed', action: 'Referral Added', details: '2 new referrals added', type: 'REFERRAL' },
      { id: 5, timestamp: '2024-12-29 11:00:00', user: 'Hassan Karim', action: 'Commission Earned', details: 'Commission of 250 USD earned', type: 'COMMISSION' },
      { id: 6, timestamp: '2024-12-29 11:15:30', user: 'Admin', action: 'User Suspended', details: 'User ID 6 suspended for violation', type: 'ADMIN' }
    ],
    total: 542
  });
});
...
app.get('/api/security/user-activities/:userId', verifyToken, adminOnly, (req, res) => {
  const { userId } = req.params;
...
app.get('/api/security/suspicious-activities', verifyToken, adminOnly, (req, res) => {
...
app.get('/api/security/status', verifyToken, adminOnly, (req, res) => {
  ...
  const totalLogs = Object.keys(activityLogger.logs).reduce((sum, key) => sum + activityLogger.logs[key].length, 0);
  ...
  res.json({
    ...
    status: {
      ...
      criticalThreats: suspiciousActivities.filter(a => a.severity === 'CRITICAL').length,
      highThreat: suspiciousActivities.filter(a => a.severity === 'HIGH').length,
...
app.get('/api/security/fingerprint/:userId', verifyToken, adminOnly, (req, res) => {
  const { userId } = req.params;
  const user = users[userId];
