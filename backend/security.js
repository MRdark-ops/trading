...
class ActivityLogger {
  ...
  getLogs (userId, filter = {}) {
    ...
    if (filter.startDate) {
      logs = logs.filter(
        (l) => new Date(l.timestamp) >= new Date(filter.startDate)
      )
    }

    if (filter.endDate) {
      logs = logs.filter(
        (l) => new Date(l.timestamp) <= new Date(filter.endDate)
      )
    }
...
class SecurityMonitor {
  ...
  recordFailedAttempt (email, ip) {
    ...
    console.log(
      `⚠️ Failed attempt: ${key} (${this.failedAttempts[key].count}/${this.MAX_FAILED_ATTEMPTS})`
    )
