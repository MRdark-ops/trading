...
router.get('/', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const internalMembers = await User.count({
      where: { userType: 'Internal Member' }
    })
    const externalMembers = await User.count({
      where: { userType: 'External Member' }
    })

    // Subscription stats
    const totalPaidSubscriptions = await Payment.count({
      where: { type: 'First Payment', status: 'Completed' }
    })
    const totalRenewals = await Payment.count({
      where: { type: 'Renewal', status: 'Completed' }
    })
    ...
    try {
      const topReferrers = await Referral.findAll({
        attributes: [
          'referrerId',
          [
            Sequelize.fn('COUNT', Sequelize.col('referrerId')),
            'totalReferrals'
          ]
        ],
        group: ['referrerId'],
