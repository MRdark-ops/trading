...
router.get('/', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { page = 1, limit = 20, status, userType, subscriptionStatus } = req.query;
...
router.get('/:userId', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const paidReferrals = referrals.filter(r => r.paymentStatus !== 'Not Paid');
    ...
    res.json({
      ...
      statistics: {
        ...
        firstPaymentDate: payments.find(p => p.type === 'First Payment')?.paymentDate,
...
// Change user type
router.patch('/:userId/type', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
...
// Reset user earnings
router.post('/:userId/reset-earnings', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
...
// Adjust user balance
router.post('/:userId/adjust-balance', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
...
router.post('/:userId/message', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
