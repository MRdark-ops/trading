...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const renewedReferrals = referrals.filter(
        (r) => r.paymentStatus === 'Renewed'
      ).length
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const deletedCount = await Referral.destroy({
        where: { referrerId: userId }
      })
  ...
  try {
    const topReferrers = await User.findAll({
      ...
      order: [
        [
          Sequelize.literal(
            '(SELECT COUNT(*) FROM referrals WHERE referrals.referrer_id = "User".id)'
          ),
          'DESC'
        ]
      ],
      ...
      topReferrers.map(async (referrer) => {
        ...
        const paidReferrals = await Referral.count({
          where: {
            ...
            paymentStatus: { [Sequelize.Op.ne]: 'Not Paid' }
          }
        })
        ...
        return {
          ...
          conversionRate:
            referrals > 0
              ? ((paidReferrals / referrals) * 100).toFixed(2) + '%'
              : '0%'
        }
      })
    )
    ...
    const { count, rows } = await Referral.findAndCountAll({
      ...
      include: [
        ...
        },
        {
          model: User,
          as: 'referee',
          attributes: ['id', 'email', 'fullName', 'subscriptionStatus']
        }
      ],
      order: [['registrationDate', 'DESC']]
    })
