  ...
  try {
    ...
    const { count, rows } = await Withdrawal.findAndCountAll({
      ...
      include: [
        {
          model: User,
          attributes: ['id', 'email', 'fullName', 'currentBalance']
        }
      ],
  ...
  async (req, res) => {
    try {
      ...
      if (withdrawal.status !== 'Pending') {
        return res
          .status(400)
          .json({ error: 'Only pending withdrawals can be approved' })
      }
  ...
  async (req, res) => {
    try {
      ...
      if (withdrawal.status !== 'Approved') {
        return res
          .status(400)
          .json({ error: 'Only approved withdrawals can be marked as paid' })
      }

      // Deduct from user balance
      const user = await User.findByPk(withdrawal.userId)
      const newBalance =
        parseFloat(user.currentBalance) - parseFloat(withdrawal.amount)

      await user.update({
        currentBalance: newBalance,
        totalWithdrawals:
          parseFloat(user.totalWithdrawals) + parseFloat(withdrawal.amount)
      })
