...
import { Copy, ExternalLink, TrendingUp, Users, DollarSign, Zap, Target } from 'lucide-react';
...
function Dashboard() {
  ...
  const commissionData = [
    { level: 1, rate: 10, target: 10, current: dashboardData?.statistics?.directDownline || 0, earnings: 25 },
    ...
    <div className="dashboard">
      <header className="dashboard-header">
        ...
        <div className="header-badge">
          <span className={`status-badge ${user?.subscriptionStatus?.toLowerCase()}`}>
      ...
      <section className="subscription-section">
        <div className="subscription-card">
          <div className="subscription-header">
            ...
            <span className={`status-indicator ${user?.subscriptionStatus?.toLowerCase()}`}></span>
          </div>
          <div className="subscription-details">
            ...
            {user?.subscriptionExpiryDate && (
              <div className="detail-item">
                ...
                <span className="value">{new Date(user.subscriptionExpiryDate).toLocaleDateString()}</span>
              </div>
            )}
          </div>
          {user?.subscriptionStatus === 'Active' && (
            <a href="https://t.me/powerattack" target="_blank" rel="noopener noreferrer" className="telegram-btn">
      ...
      <section className="metrics-section">
        <div className="metric-card">
          ...
          <div className="metric-content">
            ...
            <h3 className="metric-value">{dashboardData?.statistics?.totalReferrals || 0}</h3>
        ...
        <div className="metric-card">
          ...
          <div className="metric-content">
            ...
            <h3 className="metric-value">${(user?.totalEarnings || 0).toFixed(2)}</h3>
        ...
        <div className="metric-card">
          ...
          <div className="metric-content">
            ...
            <h3 className="metric-value">{dashboardData?.statistics?.paidReferrals || 0}</h3>
        ...
        <div className="metric-card">
          ...
          <div className="metric-content">
            ...
            <h3 className="metric-value">${(user?.currentBalance || 0).toFixed(2)}</h3>
      ...
      <section className="referral-section">
        ...
        <div className="referral-card">
          <div className="referral-link-box">
            ...
            <button onClick={() => copyToClipboard(referralLink)} className="copy-btn">
              <Copy size={18} />
              {copied ? 'Copied!' : 'Copy'}
            </button>
          </div>
          <p className="referral-hint">Share this link to earn commissions from your referrals</p>
        </div>
      </section>
          ...
          {commissionData.map((item) => (
            <div key={item.level} className="table-row">
              ...
              <div className="col-progress">
                ...
                <span className="progress-text">{item.current}/{item.target}</span>
              </div>
              <div className="col-earnings">${item.earnings.toLocaleString()}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="chart-container">
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={commissionData}>
              ...
              <YAxis stroke="#888" />
              <Tooltip 
                contentStyle={{ background: '#1a1a1a', border: '1px solid #d4af37' }}
