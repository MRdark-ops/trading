# 🛡️ أفضل الممارسات الأمنية للموقع

## 📋 جدول المحتويات

1. [المصادقة والتفويض](#المصادقة-والتفويض)
2. [حماية البيانات](#حماية-البيانات)
3. [العمليات الآمنة](#العمليات-الآمنة)
4. [الكشف عن التهديدات](#الكشف-عن-التهديدات)
5. [الاستجابة للحوادث](#الاستجابة-للحوادث)
6. [أدلة الإدارة اليومية](#أدلة-الإدارة-اليومية)

---

## 🔑 المصادقة والتفويض

### ✅ يجب عليك:

#### 1. استخدام كلمات مرور قوية

```javascript
// ✅ الصحيح
const password = "Tr@d!ng#2024$VIP!"; // 14+ حرف، أحرف كبيرة، أرقام، رموز

// ❌ خاطئ
const password = "123456"; // ضعيفة جداً
const password = "password123"; // شائعة ومتوقعة
```

#### 2. عدم مشاركة كلمات المرور

```bash
# ❌ لا تفعل هذا أبداً
export PASSWORD="admin123456"
curl -H "Authorization: Bearer $PASSWORD" http://localhost:5001/api/admin

# ✅ استخدم متغيرات البيئة الآمنة
# في ملف .env (لا تُرسِل إلى Git)
ADMIN_PASSWORD=***
```

#### 3. تغيير كلمات المرور دورياً

```bash
# كل 30 يوم على الأقل:
# 1. اذهب إلى لوحة التحكم
# 2. اضغط على "حسابي"
# 3. اختر "تغيير كلمة المرور"
# 4. أدخل كلمة جديدة قوية

# تاريخ آخر تغيير: 2024-12-29
# التاريخ المتوقع القادم: 2025-01-28
```

#### 4. لا تستخدم نفس كلمة المرور على مواقع أخرى

```bash
# ❌ خطر جداً
AdminPassword123 → تستخدمها في:
  - موقع التداول
  - البريد الإلكتروني
  - حساب البنك

# ✅ آمن
موقع التداول: Tr@d!ng#VIP#2024$
البريد الإلكتروني: Em@il$Password#2024
البنك: B@nk$Secure#Password#2024
```

### 🔐 إدارة التوكنات

#### 1. الاحتفاظ بالتوكن آمناً

```javascript
// ❌ لا تخزن التوكن في localStorage (عرضة للـ XSS)
localStorage.setItem("token", token); // خطر!

// ✅ استخدم HttpOnly Cookies
// (أكثر أماناً من localStorage)
res.cookie("token", token, {
  httpOnly: true,
  secure: true, // HTTPS فقط
  sameSite: "strict",
});
```

#### 2. تحديث التوكن

```javascript
// التوكن ينتهي بعد 7 أيام
// تحديث تلقائي:
const tokenAge = Date.now() - tokenCreatedAt;
if (tokenAge > 6 * 24 * 60 * 60 * 1000) {
  // 6 أيام
  refreshToken();
}
```

---

## 🔒 حماية البيانات

### 1. تشفير البيانات الحساسة

```javascript
// ❌ خطر - بدون تشفير
const password = "mypassword123";
db.save({ password: password });

// ✅ آمن - مع تشفير
const bcrypt = require("bcrypt");
const hashedPassword = await bcrypt.hash(password, 10);
db.save({ password: hashedPassword });
```

### 2. عدم نشر البيانات الحساسة

```javascript
// ❌ خطر - يرسل كلمة المرور
res.json({
  user: {
    email: user.email,
    password: user.password, // لا! لا تفعل هذا أبداً
    role: user.role,
  },
});

// ✅ آمن - بدون بيانات حساسة
res.json({
  user: {
    email: user.email,
    role: user.role,
    // كلمة المرور غير مُرسلة
  },
});
```

### 3. حماية البيانات في النقل

```bash
# ❌ غير آمن - بدون تشفير
http://localhost:5001/api/auth/login

# ✅ آمن - مع HTTPS
https://tradingdz.com/api/auth/login
```

### 4. نسخ احتياطية آمنة

```bash
# ✅ قم بعمل نسخة احتياطية يومياً
# وخزنها في مكان آمن (خارج السيرفر)

# مثال: AWS S3 encrypted bucket
aws s3 cp backup.tar.gz \
  s3://secure-backups-bucket/backup-$(date +%Y%m%d).tar.gz \
  --sse AES256

# في أيام الجمعة: نسخة احتياطية كاملة
# بقية الأيام: نسخ احتياطية تزايدية
```

---

## 🛡️ العمليات الآمنة

### 1. التحقق من المدخلات (Input Validation)

```javascript
// ✅ التحقق الصحيح
const validateEmail = (email) => {
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!regex.test(email)) {
    throw new Error("Invalid email format");
  }
  if (email.length > 100) {
    throw new Error("Email too long");
  }
};

// ✅ التحقق من كلمة المرور
const validatePassword = (password) => {
  if (password.length < 8) {
    throw new Error("Password too short (minimum 8)");
  }
  if (!/[A-Z]/.test(password)) {
    throw new Error("Password must contain uppercase");
  }
  if (!/[0-9]/.test(password)) {
    throw new Error("Password must contain numbers");
  }
  if (!/[!@#$%^&*]/.test(password)) {
    throw new Error("Password must contain special chars");
  }
};
```

### 2. السماح فقط بـ HTTPS

```javascript
// في mock-server.js
app.use((req, res, next) => {
  if (process.env.NODE_ENV === "production") {
    if (req.header("x-forwarded-proto") !== "https") {
      res.redirect(`https://${req.header("host")}${req.url}`);
      return;
    }
  }
  next();
});
```

### 3. حد المحاولات (Rate Limiting)

```javascript
// ✅ معدلات الحد الموصى بها

// تسجيل الدخول: 5 محاولات في الدقيقة
// (يمنع Brute Force)

// API العام: 100 طلب في الدقيقة
// (يمنع DDoS)

// تحديث الملف الشخصي: 10 مرات في الساعة
// (يمنع الإساءة)
```

### 4. تسجيل كل النشاطات (Logging)

```javascript
// ✅ سجل:
- وقت النشاط
- عنوان IP
- المستخدم
- نوع النشاط
- النتيجة (نجاح/فشل)
- أي بيانات حساسة؟ لا!

// مثال:
{
  "timestamp": "2024-12-29T15:30:00Z",
  "ip": "192.168.1.100",
  "userId": 5,
  "action": "LOGIN",
  "result": "SUCCESS",
  "metadata": {
    "userAgent": "Mozilla/5.0...",
    "fingerprint": "a7f3e2d1..."
  }
}
```

---

## 🔍 الكشف عن التهديدات

### 1. علامات الهجوم الشائعة

```javascript
const suspiciousPatterns = {
  // SQL Injection
  sqlInjection: /"'|--|;|\/\*|\*\/|xp_|sp_/i,

  // XSS
  xss: /<script|onclick|<iframe|eval\(/i,

  // Path Traversal
  pathTraversal: /\.\.\\/i,

  // Command Injection
  commandInjection: /[;&|`$()]/,
};

// استخدام:
const isSuspicious = (input) => {
  for (let pattern in suspiciousPatterns) {
    if (suspiciousPatterns[pattern].test(input)) {
      return true; // تنبيه! مشبوه
    }
  }
  return false; // آمن
};
```

### 2. مراقبة السلوك المريب

```javascript
// ✅ اكتشف:
const suspiciousBehavior = {
  // محاولات دخول متعددة فاشلة
  multipleFailed: (user) => user.failedAttempts >= 5,

  // دخول من عناوين IP مختلفة في وقت قصير
  locationJump: (user) => {
    const lastLogin = user.lastLogin;
    const now = Date.now();
    return now - lastLogin < 60000; // أقل من دقيقة
  },

  // محاولة الوصول إلى حسابات أخرى
  unauthorizedAccess: (user, targetUserId) => {
    return user.id !== targetUserId && user.role !== "admin";
  },

  // تحويلات كبيرة غير معتادة
  unusualTransaction: (user, amount) => {
    const avgTransaction = user.averageTransaction;
    return amount > avgTransaction * 5; // 5 أضعاف المتوسط
  },
};
```

---

## 🚨 الاستجابة للحوادث

### 1. اكتشاف الهجوم

```bash
# 1️⃣ تحقق من السجلات
GET http://localhost:5001/api/security/suspicious-activities

# 2️⃣ حدد مصدر الهجوم (IP)
curl http://localhost:5001/api/security/status | grep -i "ip"

# 3️⃣ لاحظ الوقت والتفاصيل
{
  "type": "MULTIPLE_FAILED_LOGINS",
  "severity": "HIGH",
  "ip": "203.0.113.45",
  "timestamp": "2024-12-29T15:25:00Z"
}
```

### 2. احظر الهجوم فوراً

```bash
# 4️⃣ احظر IP
curl -X POST http://localhost:5001/api/security/block-ip \
  -H "Authorization: Bearer <ADMIN_TOKEN>" \
  -H "Content-Type: application/json" \
  -d '{
    "ip": "203.0.113.45",
    "duration": 86400000
  }'
```

### 3. تحليل الضرر

```bash
# 5️⃣ تحقق من الحسابات المتأثرة
curl -X GET http://localhost:5001/api/security/user-activities/USER_ID \
  -H "Authorization: Bearer <ADMIN_TOKEN>"

# 6️⃣ ابحث عن أنشطة مريبة
# - تحويلات غير معروضة
# - تغييرات على الملف الشخصي
# - تحديثات كلمات المرور
```

### 4. استعادة النظام

```bash
# 7️⃣ أخبر المستخدمين المتأثرين
Email Template:
---
الموضوع: تنبيه أمني - تم اكتشاف نشاط مريب

السلام عليكم،
تم اكتشاف محاولة دخول غير مصرح بها لحسابك في التاريخ والوقت:
[التاريخ والوقت]

الإجراءات المتخذة:
✅ تم حظر عنوان IP المشبوه
✅ تم تسجيل دخول جميع الجلسات الأخرى
✅ يجب عليك تغيير كلمة المرور الآن

الرابط: https://tradingdz.com/change-password
---

# 8️⃣ اجبر المستخدم على تغيير كلمة المرور
UPDATE users SET forcePasswordChange = true WHERE id = USER_ID;

# 9️⃣ راقب النشاط المستقبلي
# تحقق من السجلات كل ساعة لمدة 7 أيام
```

---

## 📅 أدلة الإدارة اليومية

### ✅ قائمة تفقد يومية (Daily Checklist)

```
قبل فتح الموقع كل صباح:

☐ تحقق من الحالة الأمنية
  curl http://localhost:5001/api/security/status

☐ انظر للأنشطة المريبة
  curl http://localhost:5001/api/security/suspicious-activities

☐ راجع سجلات آخر 24 ساعة
  جميع العمليات محفوظة تلقائياً

☐ تحقق من الحسابات المقفولة
  اضغط على "Users" في Admin Dashboard

☐ أعد تشغيل الخادم (إذا لزم الأمر)
  cd backend && node mock-server.js
```

### ✅ قائمة فحص أسبوعية

```
كل يوم جمعة:

☐ مراجعة شاملة للأنشطة
☐ تحليل الأنماط المريبة
☐ تحديث قائمة IPs المحظورة
☐ نسخ احتياطية كاملة
☐ مراجعة الإحصائيات الأمنية
☐ تقرير لفريق الإدارة
```

### ✅ قائمة فحص شهرية

```
أول يوم من الشهر:

☐ تغيير كلمات مرور المسؤولين
☐ مراجعة الصلاحيات
☐ تحديث السياسات الأمنية
☐ اختبار خطة الاستجابة
☐ تدريب الفريق
☐ تحديث التوثيق
```

---

## 🎯 مؤشرات الأمان الصحي

```javascript
const healthySecurityMetrics = {
  // عدد الأنشطة المريبة يجب أن يكون قليلاً جداً
  suspiciousActivities: {
    perDay: 0, // مثالي: 0
    threshold: 10, // إنذار: أكثر من 10
    critical: 50, // خطر: أكثر من 50
  },

  // معدل الفشل في تسجيل الدخول
  loginFailureRate: {
    acceptable: "1-2%",
    warning: "5%",
    critical: "10%+",
  },

  // عدد IPs المحظورة
  blockedIPs: {
    normal: "0-5",
    elevated: "5-20",
    critical: "20+",
  },

  // متوسط وقت الاستجابة
  responseTime: {
    good: "<200ms",
    acceptable: "<500ms",
    slow: ">1s",
  },
};
```

---

## 🚀 نصائح لتحسين الأمان

### 1. استخدم VPN على الشبكات العامة

```bash
# عند الوصول من مقهى أو مكان عام
# استخدم VPN دائماً
# مثل: ExpressVPN, NordVPN, Surfshark
```

### 2. تفعيل المصادقة الثنائية (2FA)

```javascript
// قريباً في النسخة القادمة
// الخطوات:
// 1. تسجيل دخول ببريد وكلمة مرور
// 2. إدخال رمز من Google Authenticator
// 3. الدخول الكامل
```

### 3. المراجعة الدورية للنشاط

```bash
# كل أسبوع: تحقق من نشاط حسابك
# ابحث عن:
# - دخول غير معروف
# - تغييرات على البيانات
# - عمليات مريبة
```

### 4. الحفاظ على البرامج محدثة

```bash
# قم بتحديث npm packages شهرياً
cd backend
npm audit
npm audit fix  # إصلاح الثغرات تلقائياً
npm update     # تحديث الحزم
```

---

## 📞 الاتصال في حالة المشاكل

```
في حالة الهجوم أو الثغرة الأمنية:

1. أوقف الخادم فوراً (إذا لزم الأمر)
   pkill node

2. اجمع الأدلة:
   - السجلات
   - لقطات الشاشة
   - معلومات الهجوم

3. أخبر الفريق التقني:
   Email: security@tradingdz.com
   Phone: +213 xx xxx xxxx
   حد أقصى للوقت: 30 دقيقة

4. لا تحاول إصلاحها بنفسك
   قد تؤدي للأسوأ
```

---

## 🎓 الخلاصة

**تذكر:**

- 🔐 الأمان أولوية #1
- 🕐 بدأ المراقبة الآن
- 📝 سجل كل شيء
- 🚨 أطلب المساعدة عند الحاجة
- 🚀 حدّث دائماً الأنظمة

**الموقع آمن الآن. شكراً لاختيارك أمان البيانات! ✅**
